...
static int frames = 0;
#if 1
static lv_color_t rgb565_to_lv_color(uint16_t color_rgb565) {
  // Convert big-endian to little-endian
    color_rgb565 = (color_rgb565 >> 8) | (color_rgb565 << 8);

    uint8_t r = (color_rgb565 >> 11) & 0x1F; // Extract red component (5 bits)
    uint8_t g = (color_rgb565 >> 5) & 0x3F;  // Extract green component (6 bits)
    uint8_t b = color_rgb565 & 0x1F;         // Extract blue component (5 bits)
...
void GIFDraw(GIFDRAW *pDraw)
{
    ...
    // 获取画布的缓冲区
    uint16_t *canvas_buf = (uint16_t*)lv_canvas_get_buf(canvas);
    ...
    // 将每行像素数据绘制到画布的缓冲区对应位置
    for (x = 0; x < iWidth; x++)
    {
        canvas_buf[y * DISPLAY_WIDTH + pDraw->iX + x] = usTemp[x]>>8 | usTemp[x]<<8;
    }
    // 判断是否绘制完成一帧图像
    if (y + 1 == pDraw->iHeight)
    {
...
void GIFDraw(GIFDRAW *pDraw){
    ...
    if (pDraw->ucHasTransparency) // if transparency used
    {
        ...
        while (x < iWidth)
        {
            ...
            if (iCount) // any opaque pixels?
            {
                ...
                {
                    canvas_buf[y * DISPLAY_WIDTH + pDraw->iX + x + i] = usTemp[i];//>>8 || usTemp[i]<<8;
                }
                x += iCount;
                iCount = 0;
            }
    ...
    else
    {
        ...
        for (int i = 0; i < iCount; i++)
        {
            canvas_buf[y * DISPLAY_WIDTH + pDraw->iX + x + i] = usTemp[i];//>>8 || usTemp[i]<<8;//usTemp[0][i];
        }
        iWidth -= iCount;
        ...
        while (iWidth > 0)
        {
            ...
            for (int i = 0; i < iCount; i++)
            {
                canvas_buf[y * DISPLAY_WIDTH + pDraw->iX + (pDraw->iWidth - iWidth) + i] = usTemp[i];//>>8 || usTemp[i]<<8;//usTemp[0][i];
            }
            iWidth -= iCount;
        }
    }
    ...
    for (x = 0; x < iWidth; x++)
    {
      canvas_buf[y * DISPLAY_WIDTH + pDraw->iX + x] = usTemp[0][x];
    }
    #endif
    // 判断是否绘制完成一帧图像
    if (y + 1 == pDraw->iHeight)
...
//1. 在画布上绘制每一个像素点，但是绘制很慢，有利于了解绘制过程
// Draw a line of image directly on the LCD
void GIFDraw(GIFDRAW *pDraw){
    uint8_t *s;
    uint16_t *usPalette;
    ...
    if (pDraw->ucHasTransparency) // if transparency used
    {
        ...
        while (x < iWidth)
        {
            ...
            if (iCount) // any opaque pixels?
            {
                for (int i = 0; i < iCount; i++)
                {
                    lv_canvas_set_px(canvas, pDraw->iX + x + i, y, rgb565_to_lv_color(usTemp[0][i]),LV_OPA_COVER);
                }
                x += iCount;
                iCount = 0;
            }
    ...
    else
    {
        ...
        for (int i = 0; i < iCount; i++)
        {
            //lv_canvas_set_px(canvas, pDraw->iX + i, y, lv_color_make((usTemp[0][i] >> 11) & 0x1F, (usTemp[0][i] >> 5) & 0x3F, usTemp[0][i] & 0x1F),LV_OPA_COVER);
            lv_canvas_set_px(canvas, pDraw->iX + i, y, rgb565_to_lv_color(usTemp[0][i]),LV_OPA_COVER);
        }
        ...
        {
            if (iWidth <= BUFFER_SIZE)
                for (iCount = 0; iCount < iWidth; iCount++) usTemp[0][iCount] = usPalette[*s++];
            else
                for (iCount = 0; iCount < BUFFER_SIZE; iCount++) usTemp[0][iCount] = usPalette[*s++];

            for (int i = 0; i < iCount; i++)
            {
                lv_canvas_set_px(canvas, pDraw->iX + (pDraw->iWidth - iWidth) + i, y, rgb565_to_lv_color(usTemp[0][i]), LV_OPA_COVER);
            }
            iWidth -= iCount;
        }
    }
